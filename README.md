...
```text
S3(소스 ZIP) ──▶ CodePipeline
                  ├─ Source : S3에서 소스 가져오기
                  ├─ Build  : 평가 게이트 실행(buildspec-test.yml → tests/eval_gate.py)
                  │            평균 점수 < 임계값(0.7) 이면 파이프라인 중단
                  └─ Deploy : 평가 통과 시에만 agentcore deploy(buildspec-deploy.yml)
```
